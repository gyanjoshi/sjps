...
jQuery(document).on('ready', function() {
	...
	/* -------------------------------------
			ANNOUNCEMENT SLIDER
	-------------------------------------- */
//	var _sj_announcementslider = jQuery('#sj-announcementslider');
//	
//		_sj_announcementslider.owlCarousel({
//			items: 2,
//			nav:true,
//			margin:20,
//			loop:true,
...
//		});
	...
	jQuery('#updatepassword').on('click', function(event) {
		var _form = jQuery('#updatepasswordform');
		_form.submit();
	})
	
	/* -----------------------------------------------
	 * HANNDLE check box checked and change event in create new journal issue form
	 */
	/* Intially hide file upload div ----------------
	 * 
	 */
	
	
	
	if(!jQuery('#uploadcovercheck').is(":checked"))
		{
			
			jQuery('#coverpageupload').hide();
		}
		
	else
		{
			
			jQuery('#coverpageupload').show();
		}
		
	
	$('#uploadcovercheck').change(function() {
        if(this.checked) {
	...
	/* -----------------------------------------------
	 * HANNDLE check box checked and change event in create new announcement form
	 */
	/* Intially hide expiry date div ----------------
	 * 
	 */
	
	
	
	if(!jQuery('#hasExpiryDateCheck').is(":checked"))
		{
			
			jQuery('#hasexpirydate').hide();
		}
		
	else
		{
			
			jQuery('#hasexpirydate').show();
		}
		
	
	$('#hasExpiryDateCheck').change(function() {
        if(this.checked) {
...
{
    var html = '<ul id=\"page-items\" class=\"sj-allcategorys sj-allcategorysvtwo\">';
    html += data;
    html += '</ul>';
    return html;
}
...
function submitFeedback(e) {
	...
	var target = e.target;	
	var _form = jQuery(target).parent().siblings('form');
	...
	$(_form).submit(function (event) {
		...
		$.each(_form.serializeArray(), function(i, field) {
			var str = field.value.replace(/\s|&nbsp;/g, '');
			
		    values[field.name] = str.replace(/<p><\/p>/gi,'').trim();
		    
		});
		
		if (values['feedbackMessage'].length <= 0) {
			event.preventDefault();
...
function submitReject(e) {
	...
	$(_form).submit(function (event) {
		...
		if (values['rejectMessage'].length <= 0) {
			event.preventDefault();
...
function validate(e) {
	var target = e.target;	
	var _form = jQuery(target).parent().parent('form');
	...
	$(_form).submit(function (event) {
			...
			if (values['emailMessage'].length <= 0) {
				event.preventDefault();
...
$("#journalIssueId").load(url, function(response, status, xhr) {
	if(response.length==0)
		alert("No Journal Issues found for this journal. Please create a journal issue or select other journal!");
});	
}